...
const InputSchema = z.object({
    ...
    disclosure: z
        ...
        .max(1000)
        .optional()
        .describe("Override the compliance disclosure footer. Omit to use the server default. " +
        "Must include NMLS number and Equal Housing Lender language."),
    tone_notes: z
        .string()
        .max(300)
        .optional()
        .describe("Optional tone guidance appended to each generation prompt, e.g. " +
...
const SYSTEM_PROMPT = "You are a licensed mortgage marketing specialist who writes FINRA/CFPB-compliant " +
...
function buildUserPrompt(article, toneNotes) {
...
export function registerGenerateContent(server) {
    server.registerTool("mlo_generate_content", {
...
Error cases:
  ...
  - "OpenAI rate limit exceeded" → wait and retry`,
        ...
        try {
            ...
            const posts = await callOpenAI([
                { role: "system", content: SYSTEM_PROMPT },
                { role: "user", content: buildUserPrompt(params.article, params.tone_notes) },
            ], params.model, 0.7);
            if (!posts.linkedin || !posts.instagram || !posts.tiktok) {
                throw new Error("Generated response is missing one or more post fields (linkedin, instagram, tiktok).");
            }
            const result = {
