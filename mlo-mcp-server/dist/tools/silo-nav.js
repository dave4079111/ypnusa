import { z } from "zod";
import { DEFAULT_SILOS } from "../constants.js";
...
function silosToMarkdown(silos) {
    const lines = ["# Silo Structure", ""];
    for (const [prefix, pillar] of Object.entries(silos)) {
...
export function registerSiloNav(server) {
    // Tool 1: Resolve a URL path → nav data
    server.registerTool("mlo_get_silo_nav", {
  ...
  - "path must be at least 1 character" → provide a valid path`,
        ...
        try {
            ...
            if (params.page_title) {
                breadcrumb.push({ label: params.page_title, url: null });
            }
    ...
    server.registerTool("mlo_list_silos", {
  ...
  "total_pages": number
        ...
        try {
            ...
            if (params.response_format === "markdown") {
                return {
                    content: [{ type: "text", text: silosToMarkdown(silos) }],
                };
            }
    ...
    server.registerTool("mlo_plan_silos", {
...
3. internal_linking_rules: array of strings describing the thematic wall rules for this architecture`;
            const result = await ai([{ role: "user", content: prompt }], "gpt-4o-mini", 0.6);
...
//# sourceMappingURL=silo-nav.js.map