...
function formatMarkdown(keywords: Keyword[], topic: string): string {
  const lines = [
    `# Keyword Scout: "${topic}"`,
    "",
...
export function registerScoutKeywords(server: McpServer): void {
  server.registerTool(
    "mlo_scout_keywords",
    {
  ...
  - "OPENAI_API_KEY environment variable is not set" → set the env var`,
    ...
    async (params: Input) => {
      try {
        ...
        let keywords = getCached<Keyword[]>(cacheKey);

        if (!keywords) {
          const result = await callOpenAI<KeywordsResponse>(
            [{ role: "user", content: buildPrompt(params.topic, params.count) }],
            "gpt-4o-mini",
            0.5
          );
